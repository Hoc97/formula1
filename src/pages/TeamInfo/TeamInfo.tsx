...
const TeamInfo = () => {
  ...
  const driverInfoQuery = useDriverInfoById(teaminfo as string);
  const driverChampsQuery = useDriverChampsById(teaminfo as string);
  const driverStatsQuery = useDriverStatsById(teaminfo as string);
  ...
  const handlePhoto = () => {
    return `${env.photoMainUrl}teams/${year}Drivers/${data.familyName}.jpg`;
  };
